...
jest.unstable_mockModule('../World.js', () => ({
  default: jest.fn(() => ({ entities: {} })), // Mock World constructor and properties
}));
jest.unstable_mockModule('../Entity.js', () => ({
  default: jest.fn(() => ({ // Mock Entity constructor
    components: {},
...
const GrowthSystem = (await import('../../Systems/GrowthSystem.js')).default;
const Entity = (await import('../Entity.js')).default;
const World = (await import('../World.js')).default; // Will be the mock
const RadiusComponent = (await import('../../Components/RadiusComponent.js')).default;
const GrowComponent = (await import('../../Components/GrowComponent.js')).default;

describe('GrowthSystem', () => {
    let growthSystem;
    let mockWorldInstance; // Renamed from mockWorld to avoid confusion with World module

    beforeEach(() => {
      // Reset mocks for World and Entity if they are stateful across tests in other files
      // For this file, instances are created in createMockEntity or new World() which uses the mock.
      World.mockClear(); // Clear mock history for the World module constructor
      Entity.mockClear(); // Clear mock history for the Entity module constructor

      growthSystem = new GrowthSystem();
      // Setup a mock world instance using the mocked World constructor
      // This instance will be passed to growthSystem.update
      mockWorldInstance = new World(); // This now uses the mocked constructor
      // We can further customize mockWorldInstance if needed, e.g., mockWorldInstance.entities = ...
    });

    // Helper to create a mock entity with specified components
    const createMockEntity = (hasGrow, hasRadius, growConfig = {}, radiusConfig = {}) => {
      const entity = new Entity(); // Uses the mocked Entity constructor
      // The mocked Entity constructor already sets up mock functions for component methods
      // We can directly add mocked components to its 'components' property
      ...
      if (hasGrow) {
        ...
        entity.components.GrowComponent = growComp; // Directly assign
        entity.hasComponent.mockImplementation(name => name === 'GrowComponent');
        entity.getComponent.mockImplementation(name => name === 'GrowComponent' ? growComp : undefined);
      }
      if (hasRadius) {
        const radiusComp = new RadiusComponent(radiusConfig.initial ?? 100);
        radiusComp.increaseRadius = jest.fn(); // Mock increaseRadius to spy on it
        entity.components.RadiusComponent = radiusComp; // Directly assign
        // Adjust mockImplementation if both components can exist
        const currentHasGrow = entity.hasComponent('GrowComponent');
        entity.hasComponent.mockImplementation(name =>
          (name === 'GrowComponent' && currentHasGrow) || name === 'RadiusComponent'
        );
        entity.getComponent.mockImplementation(name => {
          if (name === 'GrowComponent' && currentHasGrow) return entity.components.GrowComponent;
          if (name === 'RadiusComponent') return entity.components.RadiusComponent;
          return undefined;
        });
      }
      return entity;
    ...
    test('update does nothing if no entities exist', () => {
      growthSystem.update(mockWorldInstance, 0.1); // 100 ms
    ...
    test('update does not affect entities without GrowComponent', () => {
      ...
      mockWorldInstance.entities[entity.id] = entity;
      growthSystem.update(mockWorldInstance, 0.1);
    ...
    test('update increases radius when interval is met', () => {
      ...
      growthSystem.update(mockWorldInstance, 0.1); // dt = 100ms, matches interval
    ...
    test('update does not increase radius if interval is not met', () => {
      ...
      growthSystem.update(mockWorldInstance, 0.1); // dt = 100ms, interval = 200ms
    ...
    test('update handles multiple growth cycles if dt is large enough', () => {
      ...
      growthSystem.update(mockWorldInstance, 0.25); // dt = 250ms. Interval = 100ms. 2 cycles should occur.
    ...
    test('update correctly accumulates time across multiple calls', () => {
      ...
      growthSystem.update(mockWorldInstance, 0.1); // dt = 100ms. timeSinceLastGrowth = 100
      expect(radiusComp.increaseRadius).not.toHaveBeenCalled();
      expect(growComp.timeSinceLastGrowth).toBe(100);

      growthSystem.update(mockWorldInstance, 0.15); // dt = 150ms. timeSinceLastGrowth = 100 + 150 = 250
      expect(radiusComp.increaseRadius).not.toHaveBeenCalled();
      expect(growComp.timeSinceLastGrowth).toBe(250);

      growthSystem.update(mockWorldInstance, 0.1); // dt = 100ms. timeSinceLastGrowth = 250 + 100 = 350. Interval 300. One cycle.
      expect(radiusComp.increaseRadius).toHaveBeenCalledTimes(1);
     ...
     test('update handles entities with zero interval carefully (should grow every update if rate > 0)', () => {
      ...
      growthSystem.update(mockWorldInstance, 0.01); // dt = 10ms. Interval = 0.
    ...
    test('processes multiple entities correctly', () => {
      ...
      growthSystem.update(mockWorldInstance, 0.1); // dt = 100ms
